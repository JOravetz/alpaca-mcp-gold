...
class Settings:
    ...
    def validate(self) -> None:
        ...
        if (
            self.alpaca_api_key == "demo_key_please_replace"
            or self.alpaca_secret_key == "demo_secret_please_replace"
        ):
            raise ValueError(
...
try:
    ...
    if any(
        key in str(e)
        for key in ["demo_key_please_replace", "demo_secret_please_replace"]
    ):
        print(f"Warning: {e}")
