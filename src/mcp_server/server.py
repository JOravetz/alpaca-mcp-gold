...
mcp = FastMCP(name=settings.server_name, version="1.0.0")
...
async def place_market_order_tool(
    symbol: str, side: str, quantity: float, time_in_force: str = "day"
) -> Dict[str, Any]:
    """
...
async def place_stop_loss_order_tool(
    ...
    """
    return await place_stop_loss_order(
        symbol, side, quantity, stop_price, time_in_force
    )


@mcp.tool()
async def get_orders_tool(
    status: Optional[str] = None, limit: int = 50, symbols: Optional[List[str]] = None
) -> Dict[str, Any]:
    """
...
async def execute_custom_trading_strategy_tool(
    ...
    """
    return await execute_custom_trading_strategy(
        strategy_code, symbols, portfolio_context
    )
...
async def execute_portfolio_optimization_strategy_tool(
    ...
    """
    return await execute_portfolio_optimization_strategy(
        optimization_code, risk_tolerance
    )
...
async def execute_custom_analytics_code_tool(
    dataset_name: str, python_code: str, include_portfolio_context: bool = False
) -> str:
    """
    ...
    Returns:
        str: Combined stdout and stderr output from code execution
    """
    return await execute_custom_analytics_code(
        dataset_name, python_code, include_portfolio_context
    )
...
@mcp.prompt()
async def trading_strategy_workshop_prompt(
...
async def clear_portfolio_state_tool() -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "metadata": {"operation": "clear_portfolio_state"},
        }
