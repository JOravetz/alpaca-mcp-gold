...
class TestWorkflowIntegration:
    ...
    async def test_market_research_to_trading_workflow(self, real_api_test):
        ...
        try:
            order_result = await place_market_order("AAPL", "buy", 1.0)  # Small quantity
    ...
    async def test_adaptive_prompts_with_data(self, real_api_test):
        ...
        assert any(keyword in prompt_text_with_data.lower() for keyword in ["portfolio", "account", "positions", "trading", "entities"])
    ...
    async def test_strategy_workshop_adaptation(self, real_api_test):
        ...
        assert any(keyword in strategy_text.lower() for keyword in ["portfolio", "growth", "strategy", "trading"])
    ...
    async def test_memory_management_across_operations(self, real_api_test):
        ...
        assert memory_after_quote["symbols_count"] >= memory_after_positions["symbols_count"]
        ...
        try:
            ...
            assert memory_after_order["symbols_count"] >= memory_after_quote["symbols_count"]
        except Exception:
    ...
    async def test_gold_standard_pattern_compliance(self, real_api_test):
        ...
        assert "portfolio" in prompt_text.lower() or "trading" in prompt_text.lower() or "trading" in prompt_text.lower()
